---
title: 谷歌云免费层级 Compute Engine
tags: Google
abbrlink: 168fe3d
date: 2025-04-15 06:54:45
---

### [Google Cloud 免费计划](https://cloud.google.com/free/docs/free-cloud-features#compute)

具体步骤可参考这篇文章 [谷歌云Free Tier长期免费云服务器](https://luotianyi.vc/7989.html)
...
由于 _e2-micro_ 只有1G 内存，创建交换分区可以缓解内存不足的状况
```bash
...
按照免费计划创建的 _e2-micro_ 还有一个缺陷就是公网 IP 是临时性的，关机后会更换 IP 地址。这个可以用 ddns 解决，将域名托管在 cloudflare ，通过cloudflare API 动态更新域名的解析地址，以后地址再怎么变也不怕
...
[安装步骤](https://doc.crowdsec.net/u/getting_started/installation/linux) 
...
[为云主机实现网络达量停机](https://taoshu.in/unix/vnstat.html)
...
[停用带通知的结算使用情况](https://cloud.google.com/billing/docs/how-to/disable-billing-with-notifications?hl=zh-cn#functions_billing_limit-nodejs)
[监听 Pub/Sub 通知](https://cloud.google.com/billing/docs/how-to/listen-to-notifications?hl=zh-cn#test-function)

![流程图](../images/budget-alert-diagram-pubsub.png)
...
**走了一遍流程发现其中的难点在于权限**
- Pub/Sub 服务 (`service-<PROJECT_NUMBER>@gcp-sa-pubsub.iam.gserviceaccount.com`) 需要 `Cloud Run Invoker` 权限来**触发/调用**你的函数
- 函数需要调用 Billing API 的权限： 当你的函数代码运行时，去调用 Cloud Billing API 来执行**停用结算**的操作。
